...
Phase 11F-G is the Heimserver Service Gate Derivation Readiness Contract. It defines the rules and constraints required for a future producer to deterministically map `server-facts.v1`, `heimserver-service-expectation.v1`, and `heimserver-service-evidence.v1` into `heimserver-service-gate-assessment.v1`. It establishes 10 norm-setting derivation cases, adds `freshness_status` to evidence, and enforces status partition rules.
Phase 11F-H is the Heimserver Service Gate Producer In-Memory. It implements a pure derivation function over the three validated inputs to satisfy the 10 Golden Cases without performing any live/IO checks. This remains future-gated.
