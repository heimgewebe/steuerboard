...
Phase 11F-E contracts the third declared assessment-input artifact, `heimserver-service-evidence.v1` (schema + validator wiring + tests): an artifact-derived, descriptive record of which service evidence is present. It introduces a descriptive `evidence_status` and a dedicated `service_evidence_*` reason-code namespace, kept separate from the assessment's verdict vocabulary. All three direct assessment-input artifact types are now contracted; evidence-internal provenance, complete derivation rules, and producer implementation remain future-gated. Contract only — still no producer, runbook kind, CLI, Stage-D, or live check.
...
Phase 11F-G enforces the Heimserver Service Gate Derivation Readiness Contract. It guarantees that a strictly defined set of Golden Cases exactly binds inputs (`server-facts.v1`, `heimserver-service-expectation.v1`, `heimserver-service-evidence.v1`) to the expected output (`heimserver-service-gate-assessment.v1`), validated by a cross-artifact schema/contract validator. It establishes the causal semantics (host identity, join rules, status mapping, precedence) without implementing a producer.
Phase 11F-H is the future Heimserver Service Gate Producer In-Memory. It will implement a pure derivation function over the three validated inputs to satisfy the validated 11F-G case inventory without performing any live/IO checks.
