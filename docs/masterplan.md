...
Phase 11F-E contracts the third producer input, `heimserver-service-evidence.v1` (schema + validator wiring + tests): an artifact-derived, descriptive record of which service evidence is present, from which a future producer may derive `evaluated_services`. It introduces a descriptive `evidence_status` and a dedicated `service_evidence_*` reason-code namespace, kept separate from the assessment's verdict vocabulary. With it the producer preimage (server-facts + expectation + service-evidence + contract rules) references are complete (evidence-internal provenance remains future work); the derivation step and assessment integration stay future-gated. Contract only — still no producer, runbook kind, CLI, Stage-D, or live check.
Phase 11F-F integrates the evidence input into the assessment contract: `heimserver-service-gate-assessment.v1` now requires `inputs.service_evidence_ref` (same `path` + `sha256` shape as the other two refs), and all five fixtures were migrated with real hashes. The assessment preimage references are now complete within a single artifact (evidence-internal provenance remains future work); the derivation step itself stays future-gated. Contract integration only — no producer, runbook kind, CLI, Stage-D, or live check, and no status-semantics change.
