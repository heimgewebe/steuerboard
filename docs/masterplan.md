...
Phase 11F-K: artifact-derived read-only runbook integration. The sixth runbook kind `heimserver-service-gate` reuses the generic `runbook run` CLI, delegates artifact loading/derivation to 11F-I and persistence to 11F-J, writes `heimserver-service-gate-assessment.json` beside trace output, maps assessment status exactly to runbook status, and rolls the assessment back if later result/trace publication fails. No live check, specialized CLI, service mutation, repair, Runtime, or Stage-D action is added.
