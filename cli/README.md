...
- The reproduced readiness artifact must carry a `repo_toplevel` binding
  derived from the read-only preflight evidence, and `--repo-path` must
  resolve to the same git toplevel
