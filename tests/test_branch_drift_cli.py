...
def test_parser_requires_explicit_bounded_threshold():
    parser = build_parser()
    args = parser.parse_args(["inventory", "branch-drift", "--warning-threshold", "7", "--json"])
...
def test_cli_dispatches_default_config_as_none(monkeypatch, capsys):
    expected = load_json(ROOT / "examples/branch-drift/mixed.json")
    calls = []
    ...
    monkeypatch.setattr("steuerboard.cli.build_branch_drift_report", build)
    result = main(["inventory", "branch-drift", "--warning-threshold", "2", "--json"])
    assert result == 0
    assert calls == [(None, 2)]
    assert json.loads(capsys.readouterr().out) == expected
